...
each house (fewest to most). If a house has the same number of children, sort alphabetically by house
...
const sortByChildren = (charArray) => {
  // Solution code here...
  charArray.sort((max,min)=>{
    if(max.children.length === min.children.length){
      return max.house.localeCompare( min.house);
    }
    else{
      return max.children.length -min.children.length;
    }
  });
  return charArray;
};
...
expression pattern to return true if the string contains the letter 'w' in lower case or false if it
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let regex=/^[A-J]/;
  const strArr=arr.filter((ele)=>regex.test(ele));
  return strArr;
};
