...
const count = (target, input) => {
  ...
  const result = input.reduce((acc, item) => {
    return (acc += item.filter((ele) => ele === target).length);
  }, 0);
  return result;
};
...
const totalSum = (input) => {
  // Solution code here...
  const result = input.reduce((acc, item) => {
    return (acc += item.reduce((singleAcc, ele) => (singleAcc += ele), 0));
  }, 0);
  return result;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const result = input.map((item) => {
    const filterItems=item.filter((ele)=>(typeof ele === 'number' && ele %5 === 0) || ele === 0?Math.pow(2,ele):'')

    return filterItems.map((ele)=>Math.pow(2,ele))
    })
      return result;
};
...
let findShortest = (data) => {
  ...
  let result=data.reduce((acc,ele)=>{
    acc=Math.min(acc,parseInt(ele.height))
    return acc
  },Infinity)
  for(const item of data){
    if(item.height == result) result= item
  }
  return result.name
};
