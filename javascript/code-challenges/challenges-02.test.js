...
the result of raising 2 to the power of the original input element. You may choose

to complete this challenge using a for loop, for...in syntax, or for...of syntax.
...
const charCode = (arr) => {
  // Solution code here...
  const charCodeArr=arr.map(element=>element.charCodeAt());
  return charCodeArr;
};
...
If any element in the array is not a number, the resulting array should have the string  in its place.
...
const evenOdd = (arr) => {
  // Solution code here...
  const evenOddArr=arr.map(element=>element % 2 ===0?'even':element % 2 ===1?'odd':"N/A");
  return evenOddArr;
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  const newNameArr=arr.map(element=>element.ability.name);
  return newNameArr;
};
