...
const typeNum = (arr) => {
  // Solution code here...
  const newArr=arr.filter(element=>Number.parseInt(element));
  return newArr;
};
...
uses filter to return an array of all the elements in the second array that are not included
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const notIncludedArr=arr.filter(element=>!forbiddenValues.includes(element)?element:'');
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  const newArr=arr.filter(element=>element.baseStat>minBaseStat?element:'');
  return newArr;
};
...
from challenge 6. For this function, extend your solution from challenge 6 to

only return the name of the stat, rather than the entire stat object.
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  const nameStatArr=arr.filter(element=>element.baseStat>minBaseStat?element.stat.name:'').map(element=>element.stat.name);
  return nameStatArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  const newArr=arr.filter(element=>!element.children?element:'');
  return newArr;
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  const newArr=arr.filter(element=>typeof element==="number").map(element=>element % 2 !=0 ?'odd':'even');
  return newArr;
};
