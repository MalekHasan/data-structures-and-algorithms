...
const toLastNames = people => {
  // Solution code here...
  const concatName=people.map(element=>element.firstName+' '+element.lastName);
  return concatName;
};
...
const addValues = (arr) => {
  // Solution code here...
  const summation=arr.reduce((acc,item,idx)=>{
    return acc+=item;
  },0);
  return summation;
};
...
the total amount purchased. Each object contains the keys `item` and `purchasePrice` like the example.
...
const addPurchases = (arr) => {
  // Solution code here...
  const total=arr.reduce((acc,item,idx)=>{
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const arrLen=arr.reduce((acc,item,idx)=>{
    return acc+=1;
...
const returnNames = (arr) => {
  // Solution code here...
  const nameCharArr=arr.reduce((acc,item,idx)=>{
    acc.push(item.name);
    return acc;
  },[]);
  return nameCharArr;
...
const reversedString = (str) => {
  ...
  const newStr=letters.reduce((acc,item,idx)=>{
    acc.unshift(item);
    return acc;
  },[]).join('');
  return newStr;
